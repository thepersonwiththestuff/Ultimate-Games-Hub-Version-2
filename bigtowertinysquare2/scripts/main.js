  ...
  window.RuntimeInterface = class g {
    ...
    async _Init(d) {
      ...
      d.runtimeBaseUrl
        ...
        : ((k = location.origin),
          (this._runtimeBaseUrl =
            ("null" === k ? "file:///" : k) + location.pathname),
          (k = this._runtimeBaseUrl.lastIndexOf("/")),
          -1 !== k &&
            (this._runtimeBaseUrl = this._runtimeBaseUrl.substr(0, k + 1)));
    ...
    async _InitDOM(d, k) {
      ...
      var l = d.engineScripts.map((q) =>
        "string" === typeof q ? new URL(q, this._runtimeBaseUrl).toString() : q
      );
    ...
    async _OnCreateJobWorker(d) {
      ...
      return { outputPort: d, transferables: [d] };
    }
    _GetLocalRuntime() {
    ...
    _PostResultToRuntime(d, k, l) {
      ...
      this._messageChannelPort.postMessage(
        { type: "result", responseId: d, isOk: k, result: l },
        n
      );
    }
    ...
    async _MaybeGetCordovaScriptURL(d) {
      return "cordova" === this._exportType &&
        ...
        ? (d.startsWith(this._runtimeBaseUrl) &&
            (d = d.substr(this._runtimeBaseUrl.length)),
    ...
    async _ConvertDataUriToBlobs(d, k) {
      if ("object" === typeof k)
        (this._localFileBlobs[d] = new Blob([k.str], { type: k.type })),
    ...
    class extends self.DOMHandler {
      constructor(f) {
        ...
        f.AddRuntimeComponentMessageHandler(
          ...
          (k) => this._SetExportingToVideo(k)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "export-to-video-progress",
          (k) => this._OnExportVideoProgress(k)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "exported-to-video",
          (k) => this._OnExportedToVideo(k)
        );
      ...
      _OnDeviceMotion(f) {
        if (!this._isExportToVideo) {
          var g = null,
            d = f.acceleration;
          d && (g = { x: d.x || 0, y: d.y || 0, z: d.z || 0 });
          d = null;
          var k = f.accelerationIncludingGravity;
          k && (d = { x: k.x || 0, y: k.y || 0, z: k.z || 0 });
          k = null;
...
self.JobSchedulerDOM = class {
  ...
  async Init() {
    ...
    this._dispatchWorker.postMessage({ type: "_init", "in-port": c.port2 }, [
      c.port2,
    ]);
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
  ...
  async _Load() {
    try {
      ...
      (this._loadState = "failed"),
        console.error(`[Construct 3] Failed to load audio '${this._url}': `, c);
    }
  }
  IsLoaded() {
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    _OnOnlineStateChanged(c) {
      this.PostToRuntime("online-state", { isOnline: c });
    }
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    Init() {
      ...
      c.onchange = (m) => {
        this.PostToRuntime("open-dlg", { chosenPath: c.value });
        try {
          c.value = null;
        } catch (p) {}
      };
      ...
      a.onchange = (m) => {
        this.PostToRuntime("folder-dlg", { chosenPath: a.value });
        try {
          a.value = null;
        } catch (p) {}
      };
      ...
      b.onchange = (m) => {
        this.PostToRuntime("save-dlg", { chosenPath: b.value });
        try {
          b.value = null;
        } catch (p) {}
      };
      ...
      setInterval(() => {
        ...
        h !== m &&
          ((h = m),
          this.PostToRuntime("clipboard-change", { "clipboard-text": m }));
      }, 1e3);
      return Object.assign(
        { argv: nw.App.argv, "clipboard-text": h },
        this._GetWindowInfo()
      );
    }
    _GetWindowInfo() {
    ...
    _OnDrop(c) {
      ...
      this.PostToRuntime("drop", { filePaths: a });
      return !1;
    }
    _OnShowOpenDlg(c) {
      if (this._isNWjs) {
    ...
    class extends self.DOMHandler {
      ...
      _OnReady() {
        ...
        for (const a of this._GetActiveGamepads())
          this.PostToRuntime("gamepad-connected", { index: a.index, id: a.id });
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    async _OnRequestPermission(c) {
      ...
      this.PostToRuntime("permission-result", { type: c, result: a });
    }
    ...
    async _RequestMotionPermission() {
      ...
      try {
        ...
        return (
          console.warn("[Touch] Failed to request motion permission: ", c), !1
        );
      }
    }
  }
    ...
    async _OnRequestWakeLock() {
      if (!this._screenWakeLock)
        try {
          ...
          console.warn("[Construct 3] Failed to acquire screen wake lock: ", c),
    ...
    class extends self.DOMElementHandler {
      ...
      CreateElement(a, b) {
        ...
        e.addEventListener("change", () =>
          this.PostToRuntimeElement("change", a, { value: parseFloat(e.value) })
        );
        e.addEventListener("input", () =>
          this.PostToRuntimeElement("input", a, { value: parseFloat(e.value) })
        );
        b.id && (e.id = b.id);
...
("use strict");
{
  ...
  self.RuntimeInterface.AddDOMHandlerClass(
    class extends self.DOMHandler {
      ...
      checkForAdTimer() {
        PokiSDK.SDK.adTimings.requestPossible()
          ? ((this._lastAdTimer = void 0),
            this._firstAdTimerDone &&
              !c() &&
      ...
      NotifyGameplayStart() {
        this._pokiSDKLoaded &&
          ...
          this._firstAdTimerDone ||
            ((this._firstAdTimerDone = !0),
            (this._lastAdTimer = setTimeout(
              () => this.checkForAdTimer(),
              PokiSDK.SDK.adTimings.timings.startAdsAfter + 10
            ))));
      }
      NotifyGameplayStop() {
        this._pokiSDKLoaded &&
      ...
      RequestCommercialBreak() {
        ...
        const a =
          this._firstAdTimerDone && PokiSDK.SDK.adTimings.requestPossible();
        ...
        return PokiSDK.commercialBreak()
          .then(() => {
            ...
            this._firstAdTimerDone &&
              ...
              (this._lastAdTimer = setTimeout(
                () => this.checkForAdTimer(),
                PokiSDK.SDK.adTimings.timings.timeBetweenAds + 10
              ));
            return { result: !0, err: !1 };
          })
          .catch((b) => {
            console.error(b);
            return { result: !1, err: b };
          });
      }
        ...
        if ("undefined" !== typeof PokiSDK) {
          ...
          return PokiSDK.init()
            .then(() => {
              ...
              return { loaded: this._pokiSDKLoaded, adBlock: e };
            })
            .catch(() => {
